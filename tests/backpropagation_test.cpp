...
TEST_CASE("Linear regression")
{
    ...
    nn.fit(nn.weights, nn.weights, data_x, data_y, data_samples, 50, 2);
...
TEST_CASE("Linear regression with 2 inputs and 2 outptus")
{
    Input input1(2);
    ...
    nn.fit(nn.weights, nn.weights, data_x, data_y, prod_samples, 20, 1);
    for (size_t i = 0; i < nn.weights_size; i++)
    {
        printf("w[%lu]: %f\n", i, nn.weights[i]);
    }
...
TEST_CASE("Solve GuessGame with regression")
{
    ...
    nn.fit(nn.weights, nn.weights, data_x, data_y, data_samples, 10, 4);
    for (size_t i = 0; i < nn.weights_size; i++)
    {
        printf("w[%lu]: %f\n", i, nn.weights[i]);
    }

    REQUIRE(abs(nn.weights[0] - 1) < 0.01f);
    REQUIRE(abs(nn.weights[1] + 0.5) < 0.01f);
    REQUIRE(abs(nn.weights[8] - 1) < 0.01f);
    REQUIRE(abs(nn.weights[9] - 2) < 0.01f);
